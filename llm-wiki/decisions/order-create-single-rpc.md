...
- 적용 전(폴백/4왕복, replica 3) 100 동시: p95 4.3s.
- 적용 후(단일 RPC, replica 3, **워밍 상태**) 100 동시: **p95 1.0~1.5s, p50 0.8~1.2s** (성공 100/100, 중복 0, 번호 연속). ~3배 개선, 목표(1초대) 달성.
- 콜드(replica 막 스케일/커넥션 미워밍) 첫 버스트는 p95 ~3s → 행사 전 `warm-on.sh`(min 3) + 워밍업으로 회피.
- 정합성 검증: 중복 menuId 합산(2+3→5), 합산>99 거부, 없는메뉴 거부, 한글 에러 정상.
## 후속: 카운터 sequence 전환 — 가설 반증

events 행 락이 tail의 원인이라 보고 카운터를 `order_number_seq`(nextval, 락 없음)로 전환 + 정렬을 order_no 단조순으로 변경.

**실측 결과: p95 거의 변화 없음** (sequence 0.9~1.6s vs events-락 1.0~1.5s).
→ **events 행 락은 병목이 아니었음.** 락을 트랜잭션 끝까지 보유해도 insert가 빨라 직렬화 영향 미미. "락 제거=빨라짐" 가설은 실측으로 반증됨.

**진짜 병목 = Supabase free-tier 쓰기 처리량**(100 동시 × insert 3종 + 풀러/CPU 한계). 락/sequence 무관.
- 100-동시-순간 p95를 확실히 <1s로: **Supabase 컴퓨트 업그레이드(free→유료)** 가 실질 레버.
- 단 실제 바자 트래픽은 시간 분산 → 현재 p95 1~1.5s(워밍 시 <1s)로 충분. tier 업그레이드는 비용 대비 불필요 판단.

sequence 전환은 유지: 성능 중립이나 **단조 정렬키(order_no) + 락 제거 + 서버권위** 이점, 비용은 번호 gap(cosmetic). [[order-aggregate]] 동시성 절 참고.

### 교훈
병목 추정은 반드시 실측으로 검증. 락 제거 같은 "그럴듯한" 최적화도 측정 없이는 효과 단정 금지. 여기선 round-trip 축소(4→1 RPC)가 실효(4.3s→1.5s), 락 제거는 무효였음.
